...
# It should be an array with each code being an array itself, so I can use the check_guess method
...
class CodeBreaker
	...
	attr_accessor :solutions_set, :total_set, :next_guess, :last_hint
	...
	def initialize
		...
		@solution_code = [rand(6), rand(6), rand(6), rand(6)]
...
until a.last_hint == ["*", "*", "*", "*"]
	...
	puts a.total_set.length
	puts a.solutions_set.length
	a.count_potential_deletes
	a.choose_guess
	a.check_guess_with_solution
end

puts a.next_guess.inspect
puts a.solution_code
